...
var (
	...
	regexEpoch           = `[1-9][0-9]*`
	smartdogeChainID     = regexp.MustCompile(fmt.Sprintf(`^(%s)%s(%s)%s(%s)$`, regexChainID, regexEIP155Separator, regexEIP155, regexEpochSeparator, regexEpoch))
)
...
func IsValidChainID(chainID string) bool {
	...
	return smartdogeChainID.MatchString(chainID)
}
...
func ParseChainID(chainID string) (*big.Int, error) {
	...
	matches := smartdogeChainID.FindStringSubmatch(chainID)
